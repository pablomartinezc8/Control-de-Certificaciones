...
export const PrintReportModal: React.FC<PrintReportModalProps> = ({
  ...
  // Acción 1: Imprimir a través de un Iframe Aislado (Fondo 100% blanco puro sin página de fondo)
  const handleSystemPrint = () => {
    ...
    if (!success) {
      setPrintWarning(
        'El visor de tu navegador restringió la ventana de impresión directa. Utiliza el botón "Descargar Archivo PDF Directo" o "Abrir en Nueva Pestaña".'
      );
    }
  };

  // Acción 2: Descargar PDF Directo usando jsPDF con pie de página oficial en todas las hojas
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-3 sm:p-4 overflow-y-auto print-modal-overlay">
      <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full flex flex-col max-h-[92vh] text-slate-900 animate-in fade-in zoom-in-95 duration-150 print-modal-card">
        
        {/* Modal Header (Oculto en impresión) */}
        <div className="p-4 sm:p-5 border-b border-slate-200 flex items-center justify-between bg-slate-50/80 rounded-t-2xl print-hide">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                Fondo blanco garantizado, sin recortes de tablas y pie de página institucional en cada hoja.
              </p>
            </div>
          </div>
        ...
        {/* Action Toolbar (Oculto en impresión) */}
        <div className="p-4 bg-white border-b border-slate-100 flex flex-wrap items-center justify-between gap-3 print-hide">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              title="Abrir el cuadro de diálogo de impresión de la computadora con fondo blanco puro y sin fondo web"
            >
            ...
            <button
              ...
              title="Generar y descargar archivo PDF en tu equipo inmediatamente con pie de página en cada hoja"
            >
        ...
        {pdfSuccess && (
          <div className="mx-4 mt-3 p-3 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs flex items-center gap-2 print-hide">
        ...
        {printWarning && (
          <div className="mx-4 mt-3 p-3 rounded-xl bg-amber-50 border border-amber-200 text-amber-800 text-xs flex items-center gap-2 print-hide">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 bg-slate-100/70 print:p-0 print:bg-white print:overflow-visible">
          <div className="text-[11px] font-bold text-slate-500 mb-2 uppercase tracking-wider flex items-center justify-between print-hide">
          ...
          <div 
            ...
            className="bg-white p-6 sm:p-8 rounded-xl shadow-xs border border-slate-200 space-y-5 w-full max-w-3xl mx-auto text-slate-900 print:max-w-none print:w-full print:p-0 print:border-none print:shadow-none"
          >
            {/* 1. Encabezado Membretado Oficial */}
            <div className="border-b-2 border-slate-900 pb-3 flex flex-col sm:flex-row sm:items-start justify-between gap-3">
              <div className="flex items-center gap-3">
            ...
            <div className="bg-slate-50 rounded-xl p-3 sm:p-4 border border-slate-200 text-xs grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              <div>
            ...
            <div className="print-table-section" style={{ pageBreakInside: 'avoid', breakInside: 'avoid' }}>
              <h4 className="text-xs font-bold uppercase tracking-wider text-slate-900 mb-1.5 flex items-center gap-1.5 border-b border-slate-200 pb-1">
              ...
              <table className="w-full text-xs border-collapse border border-slate-200">
                <thead>
                  <tr className="bg-slate-100 text-slate-700 font-bold text-[10px] uppercase">
                    <th className="border border-slate-200 py-1.5 px-3 text-left">Concepto Financiero</th>
                    <th className="border border-slate-200 py-1.5 px-3 text-right">Monto (USD)</th>
                    <th className="border border-slate-200 py-1.5 px-3 text-right">% S/ Efectivo</th>
                  </tr>
                </thead>
            ...
            <div className="print-table-section" style={{ pageBreakInside: 'avoid', breakInside: 'avoid' }}>
              ...
              <div className="grid grid-cols-3 gap-2.5 text-center text-xs">
                <div className="border border-slate-200 rounded-lg p-2 bg-slate-50">
                  ...
                  <span className="text-sm font-black text-blue-700">{avancePlan.toFixed(1)}%</span>
                  <span className="text-[9.5px] text-slate-500 font-mono block mt-0.5">{formatCurrency(montoPlanificado)}</span>
                </div>
                <div className="border border-slate-200 rounded-lg p-2 bg-slate-50">
                  <span className="text-[10px] text-slate-500 font-bold block uppercase">Real Certificado</span>
                  <span className="text-sm font-black text-emerald-700">{avanceReal.toFixed(1)}%</span>
                  <span className="text-[9.5px] text-slate-500 font-mono block mt-0.5">{formatCurrency(montoCertificado)}</span>
                </div>
                <div className={`border rounded-lg p-2 ${desvio >= 0 ? 'bg-teal-50 border-teal-200 text-teal-900' : 'bg-amber-50 border-amber-200 text-amber-900'}`}>
                  <span className="text-[10px] font-bold block uppercase">Desvío Acumulado</span>
                  <span className="text-sm font-black">{desvio >= 0 ? `+${desvio.toFixed(1)}%` : `${desvio.toFixed(1)}%`}</span>
                  <span className="text-[9.5px] block mt-0.5 font-medium">{desvio >= 0 ? 'En fecha' : 'Atraso'}</span>
                </div>
              </div>
            </div>
            ...
            {vencidos.length > 0 && (
              <div className="print-avoid-break print-table-section" style={{ pageBreakInside: 'avoid', breakInside: 'avoid' }}>
                <h4 className="text-xs font-bold uppercase tracking-wider text-rose-900 mb-1.5 flex items-center gap-1.5 border-b border-rose-200 pb-1">
                  ...
                  <tbody>
                    {vencidos.map((v) => (
                      <tr key={v.id} className="hover:bg-rose-50/40">
                        ...
                        <td className="border border-slate-200 py-1 px-2 text-center font-mono text-slate-600">
                          {v.fechaPrevista}
                        </td>
                        <td className="border border-slate-200 py-1 px-2 text-right font-mono font-bold text-rose-700">
                          {formatCurrency(v.saldoPendiente)}
                        </td>
                        <td className="border border-slate-200 py-1 px-2 text-center text-rose-800 font-bold">
            ...
            {/* 6. Próximas Entregas Programadas (Protegida contra cortes entre hojas) */}
            {proximos.length > 0 && (
              <div 
                className="print-keep-together print-table-section print-avoid-break" 
                  ...
                  <tbody>
                    {proximos.slice(0, 6).map((p) => (
                      <tr key={p.id}>
                        ...
                        <td className="border border-slate-200 py-1 px-2 text-right font-mono font-bold text-blue-900">
            ...
            {/* 7. Bloque de Firmas y Validación (No se separa) */}
            <div 
              className="signatures-section print-avoid-break pt-5 border-t border-slate-200 grid grid-cols-2 gap-8 text-center text-xs text-slate-500"
              style={{ pageBreakInside: 'avoid', breakInside: 'avoid' }}
            >
              <div className="space-y-4">
                <div className="border-b border-slate-300 w-48 mx-auto h-7" />
                <div>
            ...
            {/* Pie de página institucional visible en vista previa */}
            <div className="text-[9px] text-slate-400 text-center pt-2 border-t border-slate-100 print:hidden">
        ...
        {/* Modal Footer */}
        <div className="p-4 bg-slate-50 border-t border-slate-200 flex items-center justify-between rounded-b-2xl print-hide">
